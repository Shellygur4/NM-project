...
def create_tbss_all(name='tbss', estimate_skeleton=True, base_dir=os.getcwd()):
    ...
    >>> tbss_wf.inputs.inputnode = base_dir

ALL = tbss.create_tbss_all('tbss', estimate_skeleton=False, 'base_dir')    >>> tbss_wf = ALL
    >>> tbss_wf.inputs.inputnode.skeleton_thresh = 0.2
    >>> tbss_wf.inputs.inputnode.fa_list = ['s1_wrapped_FA.nii', 's2_wrapped_FA.nii', 's3_wrapped_FA.nii']
    >>> tbss_wf.inputs.inputnode = base_dir
    ...
    inputnode = pe.Node(
        interface=util.IdentityInterface(
            fields=['fa_list', 'skeleton_thresh', 'base_dir']),
        name='inputnode')
    ...
    tbss_all = pe.Workflow(name=name, base_dir=base_dir)
    tbss_all.connect(
...
def create_tbss_non_FA(base_dir=os.getcwd()):
    ...
    inputnode = pe.Node(
        interface=util.IdentityInterface(fields=[
            ...
            'meanfa_file', 'distance_map', 'all_FA_file', 'base_dir']),
        name='inputnode')
